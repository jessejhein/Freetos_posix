 ...
 * \file
 * UART Driver
 * \author Sam Hui <sam_hui@amonics.com>
...
#if (UART2_RX_BUF_SIZE == 0)
  #define UART2_RX_ENABLE                       0
#else /* UART2_RX_BUF_SIZE > 0 */
  #define UART2_RX_ENABLE                       1
#endif /* UART2_RX_BUF_SIZE > 0 */
#if (UART2_TX_BUF_SIZE == 0)
  #define UART2_TX_ENABLE                       0
#else /* UART2_TX_BUF_SIZE > 0 */
  #define UART2_TX_ENABLE                       1
#endif /* UART2_TX_BUF_SIZE > 0 */


//------------------------------------------------------------------------
// allocated TX/RX buffers for UART0 (UART_A) as configured
...
// allocated TX/RX buffers for UART1 (UART_B) as configured
...
// allocated TX/RX buffers for UART2 (UART_C) as configured
#if (UART2_RX_ENABLE == 1)
/** UART2 RX buffer */
__u8 uart2_rx_buf[UART2_RX_BUF_SIZE];
#endif /* UART2_RX_ENABLE */
#if (UART2_TX_ENABLE == 1)
/** UART2 TX buffer */
__u8 uart2_tx_buf[UART2_TX_BUF_SIZE];
#endif /* UART2_TX_ENABLE */
...
{
  //map pins for UART modules (PPS). Required on dsPic33E platform
  uart_pin_init ();
      ...
      case 2:
        {
          ...
          uart[2].rx_wr = 0;
          uart[2].rx_rd = 0;
          uart[2].rx_buf = &uart2_rx_buf[0];
          uart[2].rx_buf_size = UART2_RX_BUF_SIZE;
  #endif /* UART2_RX_ENABLE */
          //=====================================================
  #if (UART2_TX_ENABLE == 1)
          uart[2].tx_wr = 0;
          uart[2].tx_rd = 0;
          uart[2].tx_done = 1;
          uart[2].tx_buf = &uart2_tx_buf[0];
          uart[2].tx_buf_size = UART2_TX_BUF_SIZE;
  #endif /* UART2_TX_ENABLE */
          //=====================================================
          uart[2].io_flag = (__u8) flags;
          //=====================================================
          uart[2].cflag = 0;
          //[8-bit]
          uart[2].cflag &= ~CSIZE;
          ...
          uart[2].cflag &= ~CSTOPB;
  #endif /** 1_STOP_BIT */
          uart[2].cflag |= (0x1F & (__u16)(UART2_BAUDRATE / BAUDRATE_FACTOR));
          uart2_init ();
          //=====================================================
          break;
        }
...
uart0_init (void)
{
  ...
  if (uart[0].cflag & PARENB)
    {
      //Odd Parity
      if (uart[0].cflag & PARODD) U1MODEbits.PDSEL = 2;
      //Even Parity
...
uart1_init (void)
{
  ...
  if (uart[1].cflag & PARENB)
    {
      //Odd Parity
      if (uart[1].cflag & PARODD) U2MODEbits.PDSEL = 2;
      //Even Parity
...
uart2_init (void)
{
  ...
  //=============================================================================
  // Configure Baud rate
  __u32 speed = (uart[2].cflag & 0x1F) * (__u32)BAUDRATE_FACTOR;
  U3BRG = GEN_UART_BAUDRATE (speed);
  //==============================================================================
  // Configure Mode [Default: 8N1]
  U3MODE = 0x0000;
  if (uart[2].cflag & PARENB)
    {
      //Odd Parity
      if (uart[2].cflag & PARODD) U3MODEbits.PDSEL = 2;
      //Even Parity
      else U3MODEbits.PDSEL = 1;
    }
  if (uart[2].cflag & CSTOPB)
    {
      //2 Stop bits
      U3MODEbits.STSEL = 1;
    }
  ...
  // Configure Interrupt
#if (UART2_RX_ENABLE == 1)
  _U3RXIF = 0;                //Clear RX interrupt flags
  _U3RXIE = 1;                //Receive interrupt: 0 disable, 1 enable
  U3STAbits.URXISEL0 = 0;     //RX Interrupt when a character is received
  U3STAbits.URXISEL1 = 0;
#endif /* UART2_RX_ENABLE */
#if (UART2_TX_ENABLE == 1)
  _U3TXIF = 0;                //Clear TX interrupt flags
  _U3TXIE = 1;                //Transmit interrupt: 0 disable, 1 enable
  U3STAbits.UTXISEL0 = 1;     //TX Interrupt when all TX is completed
...
#if (UART0_TX_ENABLE == 1)
/**
 * \brief Transmit Interrupt Handler for dsPic33E U1 module used by U0ART (UART_A)
...
_U1TXInterrupt (void)
{
  ...
  if (next != CIRBUF_RD_EMPTY)
    {
      U1TXREG = (uart[0].tx_buf[uart[0].tx_rd] & 0xFF);
      uart[0].tx_rd = next;
    }
...
#if (UART1_TX_ENABLE == 1)
/**
 * \brief Transmit Interrupt Handler for dsPic33E U2 module used by U1ART (UART_B)
 * \remarks TX Interrupt when all TX is completed
 * \remarks First interrupt is set manually in uart_write
 */
void _IRQ
_U2TXInterrupt (void)
{
  ...
  if (next != CIRBUF_RD_EMPTY)
    {
      U2TXREG = (uart[1].tx_buf[uart[1].tx_rd] & 0xFF);
      uart[1].tx_rd = next;
    }
  //no more data to transmit
  else
...
#if (UART2_TX_ENABLE == 1)
/**
 * \brief Transmit Interrupt Handler for dsPic33E U3 module used by U2ART (UART_C)
 * \remarks TX Interrupt when all TX is completed
 * \remarks First interrupt is set manually in uart_write
 */
void _IRQ
_U3TXInterrupt (void)
{
  _U3TXIF = 0;
   ...
   __u8 next = cirbuf_rd (uart[2].tx_wr, uart[2].tx_rd, uart[2].tx_buf_size);
  //data available to transmit
  if (next != CIRBUF_RD_EMPTY)
    {
      U3TXREG = (uart[2].tx_buf[uart[2].tx_rd] & 0xFF);
      uart[2].tx_rd = next;
    }
  //no more data to transmit
  else
    {
      while (!U3STAbits.TRMT);  //wait until the last transmission has ended
      UART_C_RS485_TX_OFF ();   //Set transmit pin to Low, i.e. back to receive mode
      uart[2].tx_done = 1;
    }
}
#endif /* UART2_TX_ENABLE */
...
#if (UART0_RX_ENABLE == 1)
/**
 * \brief Receive Interrupt Handler for dsPic33E U1 module used by U0ART (UART_A)
  ...
  while (U1STAbits.URXDA)
    {
      ...
      if (next != CIRBUF_WR_FULL)
        {
          uart[0].rx_buf[uart[0].rx_wr] = (__u8) U1RXREG;
          uart[0].rx_wr = next;
        }
...
#if (UART1_RX_ENABLE == 1)
/**
 * \brief Receive Interrupt Handler for dsPic33E U2 module used by U1ART (UART_B)
 * \remarks RX Interrupt when a character is received
 * \remarks Read until the RX buffer is emptied
 */
void _IRQ
  ...
  while (U2STAbits.URXDA)
    {
      ...
      if (next != CIRBUF_WR_FULL)
        {
          uart[1].rx_buf[uart[1].rx_wr] = (__u8) U2RXREG;
          uart[1].rx_wr = next;
        }
      //When buffer is full, still remove data from register, but the incoming data is lost
      else
...
#if (UART2_RX_ENABLE == 1)
/**
 * \brief Receive Interrupt Handler for dsPic33E U3 module used by U2ART (UART_C)
 * \remarks RX Interrupt when a character is received
 * \remarks Read until the RX buffer is emptied
 */
void _IRQ
_U3RXInterrupt(void)
{
  ...
  while (U3STAbits.URXDA)
    {
      __u8 next = cirbuf_wr (uart[2].rx_wr, uart[2].rx_rd, uart[2].rx_buf_size);
      //If buffer is not full
      if (next != CIRBUF_WR_FULL)
        {
          uart[2].rx_buf[uart[2].rx_wr] = (__u8) U3RXREG;
          uart[2].rx_wr = next;
        }
      //When buffer is full, still remove data from register, but the incoming data is lost
      else
...
#if (UART2_TX_ENABLE == 1)
          uart[device].tx_done = 0;     //start new transmission
          UART_C_RS485_TX_ON ();        //Set transmit pin to High
          _U3TXIF = 1;                  //Start interrupt
#endif /* UART2_TX_ENABLE */
        }
      return byte;
    }
      ...
      case 2:
        {
          ...
          UART_C_RS485_TX_OFF ();       //Set TX pin to Low, i.e. back to receive mode
          _U3TXIF = 0;                  //Clear TX interrupt flag
          _U3TXIE = 0;                  //Disable TX interrupt
          U3STAbits.UTXEN = 0;          //Disable TX
          uart[2].tx_done = 1;          //Set TX as completed
#endif /* UART2_TX_ENABLE */

#if (UART2_RX_ENABLE == 1)
          //Wait for RX to complete
          while (!U3STAbits.RIDLE);
          _U3RXIF = 0;                  //Clear RX interrupt flag
          _U3RXIE = 0;                  //Disable RX interrupt
#endif /* UART2_RX_ENABLE */
          U3MODEbits.UARTEN = 0;
          break;
        }
          ...
          switch (device)
            {
              ...
              case 2:
                {
                  uart[2].cflag = ((struct termios*)argp)->c_cflag;
                  uart2_init ();
                  break;
                }
#endif /* NO_OF_UART > 2 */
          ...
          switch (device)
            {
              ...
              case 2:
                {
                  ((struct termios*)argp)->c_cflag = uart[2].cflag;
                  break;
                }
#endif /* NO_OF_UART > 2 */
              //UART id not recognised
