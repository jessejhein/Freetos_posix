 ...
 * \verbatim 
   ...
   \endverbatim 
 * \li FFT computation data space must be resided in Y-memory and properly aligned
 * \li FFT computation of 512 data points requires about 10ms (running at 40MIPS)
 */

#if (FFT_BLOCK_LENGTH == 64)
const _PROG(FFT_BLOCK_LENGTH*2) complex_t twiddleFactors[] =
{
...
#else
#error "the defined FFT_BLOCK_LENGTH is not supported."
#endif /* FFT_BLOCK_LENGTH */


/** store input sample data */
complex_t _YMEM(FFT_BLOCK_LENGTH * 2 * 2) fft_data[FFT_BLOCK_LENGTH];


/**
 * \remarks previously assigned FFT data will be overwritten
 */
void
fft_set_data (int index, real_t value)
{
  if (index < FFT_BLOCK_LENGTH)
    {
      //scale data points from [-1.0, 1.0] to [-0.5, 0.5]
      value = value >> 1;
      //assigned to ymemory in complex form
      fft_data[index].real = (fractional) value;
      fft_data[index].imag = 0x0000;
    }
}
...
fft (void)
{
  ...
  FFTComplexIP (FFT_LOG2_BLOCK_LENGTH, (fractcomplex *) &fft_data[0], 
                  (fractcomplex *) __builtin_psvoffset(&twiddleFactors[0]), 
                  (int) __builtin_psvpage(&twiddleFactors[0]));

  //Store output samples in bit-reversed order of their addresses
  return (complex_t*) BitReverseComplex (FFT_LOG2_BLOCK_LENGTH, (fractcomplex *) &fft_data[0]);
}


/**
 * \remarks the source data will be overwritten
 */
real_t* 
fft_sq_magnitude (complex_t* data, int* index)
{
  //Compute the square magnitude of the complex FFT output array so we have a Real output vetor
  real_t* result = SquareMagnitudeCplx(FFT_BLOCK_LENGTH, (fractcomplex *) &data[0], (fractional *) &data[0].real);

  // Find the frequency Bin ( = index into the SigCmpx[] array) that has the largest energy
  //i.e., the largest spectral component
  fractional maxValue = VectorMax(FFT_BLOCK_LENGTH/2, &data[0].real, index);
