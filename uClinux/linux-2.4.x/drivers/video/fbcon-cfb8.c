...
/*#if defined(CONFIG_BOARD_MBA44)
    0x00000000,0x00ff0000,0xff000000,0xffff0000,
    0x0000ff00,0x00ff00ff,0xff0000ff,0xffffff00,
    0x000000ff,0x00ffff00,0xff00ff00,0xffff00ff,
		     ...
		     int xx)
{
    ...
    case 8:
	for (rows = fontheight(p) ; rows-- ; dest += bytes) {
	    fb_writel((nibbletab_cfb8[*cdat >> 8])/* & eorx) ^ bgx*/, dest);
	    fb_writel((nibbletab_cfb8[*cdat++ & 0xff])/* & eorx) ^ bgx*/, dest+4);
        }
        break;
    case 12:
    case 16:
