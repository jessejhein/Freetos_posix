...
static int base_addr	 = 0xa000000; //0xd3800000;
static int data_reg_addr = 0xa000004; //0xd3810000;
static int irq		 = 24; //34;		/* Also change SL811_SET_GPDR! */
static int gprd 	 = 24; //13;		/* also change irq  !!! */

MODULE_PARM (base_addr, "i");
MODULE_PARM_DESC (base_addr, "sl811 base address 0xa000000"); //0xd3800000");
MODULE_PARM (data_reg_addr, "i");
MODULE_PARM_DESC (data_reg_addr, "sl811 data register address 0xa000004"); //0xd3810000");
MODULE_PARM (irq, "i");
MODULE_PARM_DESC (irq, "IRQ 24"); //"IRQ 34 (default)");
MODULE_PARM(gprd,"i");
MODULE_PARM_DESC(gprd,"sl811 GPRD port 24");  //13(default)");
...
static void inline init_irq (void)
{
	...
	outl((inl(S3C44B0X_EXTINT) & 0xffffff0f) | 0x00000040, S3C44B0X_EXTINT);
        //outl(inl(S3C44B0X_INTMSK) & ( ~( 1 << gprd ) ),S3C44B0X_INTMSK );	
}
